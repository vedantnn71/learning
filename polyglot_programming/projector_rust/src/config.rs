...
impl TryFrom<Vec<String>> for Operation {
    ...
    fn try_from(mut value: Vec<String>) -> Result<Self> {
        ...
        if term == "add" {
            if value.len() != 3 {
                let err = anyhow!("operation add expects 2 args but got {}", value.len() - 1);
                return Err(err);
            }
        ...
        if value.len() > 1 {
            let err = anyhow!("operation print expects 0 or 1 arguments but got {}", value.len() - 1);
            return Err(err);
        }
...
fn get_config(config: Option<PathBuf>) -> Result<PathBuf> {
    ...
    // ? -> unwraps
    let location = std::env::var("XDG_CONFIG_HOME")
...
fn get_pwd(pwd: Option<PathBuf>) -> Result<PathBuf> { 
    ...
    return Ok(std::env::current_dir().context("can't get current directory")?);
}
