...
func getArgs(opts *Opts) ([]string, error) {
  ...
  if operation == Add {
    if len(opts.Args) != 3 {
      return nil, fmt.Errorf("add requires 2 args, but recieved %v", len(opts.Args))
    }
  ...
  if operation == Remove {
    if len(opts.Args) != 2 {
      return nil, fmt.Errorf("add requires 1 args, but recieved %v", len(opts.Args))
    }
  ...
  if len(opts.Args) > 1 {
    return nil, fmt.Errorf("print requires - or 1 arguments but got %v", len(opts.Args))
  }
